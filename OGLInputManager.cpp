...
void OGLInputManager::addKey(std::string keyName, unsigned char key)
{
...
void OGLInputManager::updateKey(unsigned int key)
{
	this->mKeyboardInput->update(key);
}

bool OGLInputManager::isKeyPressed(std::string keyName)
{
...
int OGLInputManager::getLastMouseX(void)
{
	return(this->mMouseInput->getLastX());
}

int OGLInputManager::getLastMouseY(void)
{
	return(this->mMouseInput->getLastY());
}
